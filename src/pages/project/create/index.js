...
const [listingTypeList, setListingTypeList] = React.useState([]);
...
const formik = useFormik({
  ...
  onSubmit: (values) => {
    if (type == "ADD") {
      // const taskId = `${gender.code}${listing.code}`
...
await axio
  ...
  .then((genderResponse) => {
    console.log("genderResponse", genderResponse);
    setGenderList(genderResponse.data.payload.data);
  })
  .catch((err) => {
    console.log("brandError", err);
  });
...
await axio
  ...
  .then((marketplaceResponse) => {
    ...
    setMarketList(marketplaceResponse.data.payload.data);
  })
  .catch((err) => {
    console.log("marketplaceError", err);
  });
};
useEffect(() => {
getMarketList();
...
// getlistinglist
const getListingTypeList = async () => {
console.log("analystData", moduleData);
...
await axio
  ...
  .then((listingTypeResponse) => {
    console.log("listingTypeResponse", listingTypeResponse);
    setListingTypeList(listingTypeResponse.data.payload.data);
  })
  .catch((err) => {
    console.log("marketError", err);
  });
};
useEffect(() => {
getListingTypeList();
...
return (
  ...
  >
    <FormControl className={classes.formControl} fullWidth>
      ...
      >
        {marketList.map((marketPlace) => (
          <MenuItem key={marketPlace._id} value={marketPlace._id}>
            <Checkbox
              checked={formik.values.marketPlace.indexOf(marketPlace._id) > -1}
            />
...
>
  <FormControl className={classes.formControl} fullWidth>
    {type === "EDIT" ? (
      <>
        <InputLabel
          id="market-place-label"
          style={{ left: 12, top: -30, fontSize: 11 }}
        >
          Market Place
        </InputLabel>
        <TextField
          id="marketPlace"
          value={formik.values.marketPlace?.map((market) => market.name).join(', ') || ""}
          variant="outlined"
          fullWidth
          disabled
          InputProps={{
      ...
      <>
        <InputLabel id="market-place-label">Market Place</InputLabel>
        <Select
          labelId="market-place-label"
          id="marketPlace"
          variant="outlined"
          multiple
          ...
          renderValue={(selected) =>
            marketList
              .filter((marketPlace) => selected.includes(marketPlace._id))
        ...
        >
          {marketList.map((marketPlace) => (
            <MenuItem key={marketPlace._id} value={marketPlace._id}>
              <Checkbox
                checked={
                  formik.values.marketPlace?.indexOf(marketPlace._id) > -1
                }
              />
              <ListItemText primary={marketPlace.name} />
            </MenuItem>
...
{type === "EDIT" ? (
  ...
  <TextField
    id="gender"
    value={formik.values.gender?.name || ''}
    variant="outlined"
    fullWidth
    disabled
    InputProps={{
  ...
  <>
    ...
    >
      {genderList?.map((gender) => (
        <MenuItem key={gender._id} value={gender._id}>
          {gender.name}
        </MenuItem>
      ))}
    </Select>
    ...
    <>
      ...
      <TextField
        ...
        value={
          ...
          )?.label || formik.values.articleType?.label 
        }
        readOnly 
        variant="outlined"
        fullWidth
...
{type === "EDIT" ? (
  <>
  <InputLabel id="listing-label" style={{ left: 12, top: -30 , fontSize:11}}>
  Select Listing Type
</InputLabel>
  
  <TextField
    id="listingType"
    value={formik.values.listingType?.name || ''}
    variant="outlined"
    fullWidth
    disabled
    InputProps={{
  ...
  <>
    <InputLabel id="listingType-label" style={{ left: 12, top: -10 }}>
      Select Listing
    </InputLabel>
    <Select
      id="listingType"
      ...
      onChange={formik.handleChange}
      onBlur={formik.handleBlur}
      error={Boolean(formik.errors.listingType && formik.touched.listingType)}
      {...formik.getFieldProps("listingType")}
    >
      {listingTypeList?.map((listingType) => (
        <MenuItem key={listingType._id} value={listingType._id}>
          {listingType.name}
        </MenuItem>
      ))}
    </Select>
